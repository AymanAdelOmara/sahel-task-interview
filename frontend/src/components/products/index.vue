<template>
    <div>
        ...
        <div class="row sidemenu-height">
            <div class="col-xl-4 col-md-4" v-for="item in items" :key="item.id">
                <div class="card custom-card">
                    <img class="card-img-top w-100 blog-img" style="height:200px" :src="$imagePath + '/media/4.jpg'" alt="">
                    <div class="card-body">
                        <h4 class="card-title">{{item.name}} - this.$jQUery{{item.price}}</h4>
                        <hr>
    ...
    export default {
        ...
        watch: {
            selectedCountry: function(newVal) {
                this.getItems(`country_id=this.${newVal}`, false);
            },
        },
        components: {
        },
        ...
        methods: {
            ...
            editItem(product) {
                this.$router.push({path: `/admin/products/this.${product.id}`}).then().catch();
            },
            addItem(item) {
