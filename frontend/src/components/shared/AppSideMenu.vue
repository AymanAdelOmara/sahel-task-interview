...
export default {
    name: "AppSideMenu",
    mounted() {
        const body = this.$jQUery('body');
        this.$jQUery('.main-sidebar .with-sub').on('click', function (e) {
            e.preventDefault();
            this.$jQUery(this).parent().toggleClass('show');
            this.$jQUery(this).parent().siblings().removeClass('show');
        });
        this.$jQUery(document).on('click touchstart',  (e) => {
            e.stopPropagation();
            if (!this.$jQUery(e.target).closest('.main-header-menu-icon').length) {
                const sidebarTarg = this.$jQUery(e.target).closest('.main-sidebar').length;
                if (!sidebarTarg) {
                    body.removeClass('main-sidebar-show');
                }
            }
        });
        this.$jQUery(document).on('click', '#mainSidebarToggle', function (event) {
